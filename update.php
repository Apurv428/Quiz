<?php
  ...
  if(@$_GET['q']== 'quiz' && @$_GET['step']== 2) 
  {
    ...
    if($ans == $ansid)
    {
      ...
      $r++;
      $s=$s+$sahi;
      $q=mysqli_query($con,"UPDATE `history` SET `score`=$s,`level`=$sn,`sahi`=$r  WHERE  email = '$email' AND eid = '$eid'")or die('Error124');
    } 
    else
    {
      ...
      $w++;
      $s=$s-$wrong;
      $q=mysqli_query($con,"UPDATE `history` SET `score`=$s,`level`=$sn,`wrong`=$w WHERE  email = '$email' AND eid = '$eid'")or die('Error147');
    }
    if($sn != $total)
    {
      $sn++;
